...
TEST_F(HeapChunkSelectorTest, test_identical_layout) {
    ...
    for (uint16_t pg_id = 1; pg_id < 4; ++pg_id) {
        ...
        for (int j = 3; j > 0; --j) {
            ...
            ASSERT_TRUE(HCS.release_virtual_chunk(pg_id, v_chunkID.value(), j));
            ASSERT_EQ(HCS.m_chunks[p_chunk_id]->m_state, ChunkState::AVAILABLE);
            ...
            // local_create_shard commit binds the owner via acquire_virtual_chunk (SELECTED -> INUSE).
            ASSERT_NE(HCS.select_chunk(count, hints), nullptr);
            ASSERT_EQ(HCS.m_chunks[p_chunk_id]->m_state, ChunkState::SELECTED);
            ASSERT_EQ(pg_chunk_collection->available_num_chunks, j - 1);
            ASSERT_EQ(pg_chunk_collection->available_blk_count, start_available_blk_count - j);

            ASSERT_NE(HCS.acquire_virtual_chunk(pg_id, v_chunkID.value(), static_cast< homeobject::shard_id_t >(j)),
                      nullptr);
            ASSERT_EQ(HCS.m_chunks[p_chunk_id]->m_state, ChunkState::INUSE);
            ASSERT_EQ(pg_chunk_collection->available_num_chunks, j - 1);
            ASSERT_EQ(pg_chunk_collection->available_blk_count, start_available_blk_count - j);
...
TEST_F(HeapChunkSelectorTest, test_acquire_virtual_chunk_and_release_virtual_chunk) {
    for (uint16_t pg_id = 1; pg_id < 4; ++pg_id) {
        const homeobject::shard_id_t owner = 0x1000 + pg_id;
        const homeobject::shard_id_t other_owner = 0x2000 + pg_id;
        // test fake
        ASSERT_FALSE(HCS.release_virtual_chunk(FAKE_PG_ID, FAKE_CHUNK_ID, owner));
        ASSERT_FALSE(HCS.release_virtual_chunk(pg_id, FAKE_CHUNK_ID, owner));
        ASSERT_EQ(nullptr, HCS.acquire_virtual_chunk(FAKE_PG_ID, FAKE_CHUNK_ID, owner));
        ASSERT_EQ(nullptr, HCS.acquire_virtual_chunk(pg_id, FAKE_CHUNK_ID, owner));
        ASSERT_FALSE(HCS.check_virtual_chunk(FAKE_PG_ID, FAKE_CHUNK_ID, owner));
        ASSERT_FALSE(HCS.check_virtual_chunk(pg_id, FAKE_CHUNK_ID, owner));
        ...
        ASSERT_TRUE(HCS.check_virtual_chunk(pg_id, v_chunk_id, owner));

        // acquire an AVAILABLE chunk -> INUSE, owner set
        auto chunk = HCS.acquire_virtual_chunk(pg_id, v_chunk_id, owner);
        ...
        chunk = HCS.acquire_virtual_chunk(pg_id, v_chunk_id, owner);
        ASSERT_NE(nullptr, chunk);
        ASSERT_EQ(HCS.m_chunks[p_chunk_id]->m_state, ChunkState::INUSE);
        ASSERT_EQ(pg_chunk_collection->available_num_chunks, 2);
        ASSERT_EQ(pg_chunk_collection->available_blk_count, 1 + 2);
        ...
        ASSERT_FALSE(HCS.check_virtual_chunk(pg_id, v_chunk_id, other_owner));
        ASSERT_EQ(nullptr, HCS.acquire_virtual_chunk(pg_id, v_chunk_id, other_owner));
        ASSERT_EQ(HCS.m_chunks[p_chunk_id]->m_owner_shard_id, owner);

        // owner releases this chunk back to HeapChunkSelector
        ASSERT_TRUE(HCS.release_virtual_chunk(pg_id, v_chunk_id, owner));
        ASSERT_EQ(HCS.m_chunks[p_chunk_id]->m_state, ChunkState::AVAILABLE);
        ...
        chunk = HCS.acquire_virtual_chunk(pg_id, v_chunk_id, other_owner);
        ASSERT_NE(nullptr, chunk);
        ASSERT_EQ(HCS.m_chunks[p_chunk_id]->m_state, ChunkState::INUSE);
        ...
        ASSERT_TRUE(HCS.release_virtual_chunk(pg_id, v_chunk_id, other_owner));
    }
}
...
TEST_F(HeapChunkSelectorTest, test_gc_state_split_and_is_valid) {
    for (uint16_t pg_id = 1; pg_id < 4; ++pg_id) {
        ...
        ASSERT_NE(nullptr, HCS.acquire_virtual_chunk(pg_id, owned_v, owner));
        ...
        // restore to AVAILABLE for subsequent tests
        ASSERT_TRUE(HCS.release_virtual_chunk(pg_id, owned_v, owner));
...
TEST_F(HeapChunkSelectorTest, test_selected_state) {
    for (uint16_t pg_id = 1; pg_id < 4; ++pg_id) {
        ...
        ASSERT_NE(nullptr, HCS.acquire_virtual_chunk(pg_id, v_chunk_id, std::nullopt));
        ...
        ASSERT_NE(nullptr, HCS.acquire_virtual_chunk(pg_id, v_chunk_id, owner));
...
TEST_F(HeapChunkSelectorTest, test_recovery) {
    ...
    for (uint16_t pg_id = 1; pg_id < 4; ++pg_id) {
        ...
        ASSERT_TRUE(v_chunkID.has_value());
        auto chunk = HCS_recovery.acquire_virtual_chunk(pg_id, v_chunkID.value(), 9999);
        ASSERT_NE(chunk, nullptr);
